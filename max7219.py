# Adpoted from https://github.com/JennaSys/micropython-max7219
...
# MicroPython: SPI_BUS = 1  # hardware SPI
SPI_BAUDRATE = 100000
# MicroPython: SPI_CS = 5  # D5

class SevenSegment:
    def __init__(self, digits=8, scan_digits=MAX7219_DIGITS, baudrate=SPI_BAUDRATE, pin_clk=board.SCK, pin_cs=board.D3, pin_din=board.MOSI, reverse=False):
    # MicroPython: def __init__(self, digits=8, scan_digits=MAX7219_DIGITS, baudrate=SPI_BAUDRATE, cs=SPI_CS, spi_bus=SPI_BUS, reverse=False):
        """
        Constructor:
        ...
        `spi_bus` is the SPI bus on the controller to utilize - defaults to SPI bus 1 - not used in CircuitPython
        ...
        # MicroPython: self._spi = SPI(spi_bus, baudrate=baudrate, polarity=0, phase=0)
        self._spi = busio.SPI(pin_clk, MOSI=pin_din)
        while not self._spi.try_lock():
            pass
        self._spi.configure(baudrate=baudrate, phase=0, polarity=0)
        self._spi.unlock()

        # MicroPython: self._cs = Pin(cs, Pin.OUT, value=1)
    ...
    def _write(self, data):
        ...
        # MicroPython: self._cs.off()
        self._cs.value = False
        while not self._spi.try_lock():
            pass
        self._spi.write(bytes(data))
        self._spi.unlock()
        # MicroPython: self._cs.on()
        self._cs.value = True
